...
class NewsLetterController extends Controller
{
    ...
    public function newsStore(Request $request)
    {
        ...
        // return view('backend.news_letter.mail', compact('news'));


        // Mail::send('backend.news_letter.mail', compact('news'),

        //     function ($m) use ($news) {
        //         $customer = Cutomer::all();
        //             foreach ($customer as $data)
        //             {
        //                 $m->from(Auth::guard('admin')->user()->email, Auth::guard('admin')->user()->name);
        //                 $m->to($data->email);
        //             }
        //     });
